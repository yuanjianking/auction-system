...
class CreateScStorageTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    {
        //入库表
        Schema::create('sc_storage',function(Blueprint $table)
        {
            $table->increments('storage_number')-> comment('入库单号');
            $table->string('product_id',20)->comment('商品编号');
            $table->integer('product_qty')->default(0)->index()->comment('商品数量');
            $table->dateTime('storage_time')->comment('入库时间');
